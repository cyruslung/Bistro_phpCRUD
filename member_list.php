...
$t_sql = "SELECT COUNT(1) FROM address_book WHERE `m_level` LIKE 'member' " ;
$totalRows = $pdo->query($t_sql)->fetch(PDO::FETCH_NUM)[0]; //拿到總筆數
$totalPage = ceil($totalRows / $per_page);
$page = isset($_GET['page']) ? intval($_GET['page']) : 1; //用戶要看第幾頁
$sql = sprintf(
    "SELECT * FROM `address_book` WHERE `m_level` LIKE 'member' ORDER BY sid DESC LIMIT %s, %s",
    ($page - 1) * $per_page,
    $per_page
);
$stmt = $pdo->query($sql);
...
switch ($_SESSION['loginUser']['m_level']) {
    case 'address_book':
        $sql = sprintf(
            "SELECT * FROM `address_book` WHERE `uid`=%s ORDER BY sid DESC LIMIT %s, %s",
...
<?php include __DIR__. '/__navbar.php' ?>
    ...
    <div class="pull-left">
        <button type="button" class="btn btn-white" onclick="window.location.href='member_data_insert.php'">新增會員</button>
    </div>
    <div class="clearfix">
    ...
    <table id="data-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr> <!--不換行 class="text-nowrap"-->
                ...
                <th scope="col">#</th>
                <th scope="col">姓名</th>
                <th style="display:none" scope="col">權限</th>
                <th scope="col">性別</th>
        ...
        <?php while($r=$stmt->fetch()) { ?> <!--拿到第一筆資料 如果為true 開始執行-->
            <tr>
                ...
                <td style="display:none"><?= htmlentities($r['m_level']) ?></td>
                <td><?= htmlentities($r['m_sex']) ?></td>
                <td><?= htmlentities($r['id_card']) ?></td>
                ...
                <td>
                    <div class="action-buttons">
                        <a class="green" href="member_data_edit.php?sid=<?= $r['sid'] ?>">
        ...
        function event_pre(name_b, value) {
            if (value) {
                if (confirm(`確定要開啟${name_b}的活動功能嗎?`)) {
                    ...
                    location.href = 'member_data_block_api.php?name=' + name_b + '&value=' + '1'
                }
            } else {
                if (confirm(`確定要關閉${name_b}的活動功能嗎?`)) {
                    ...
                    location.href = 'member_data_block_api.php?name=' + name_b + '&value=' + '0'
                }
            }
        };
    </script>
    ...
    <script>
        ...
        function delete_one(sid) {
            if (confirm(`確定要刪除編號為 ${sid} 的資料嗎?`)) {
                location.href = 'member_data_delete.php?sid=' + sid;
            }
        }
        // 多重刪除 
        function delete_all() {
            ...
            if (!sids.length) {
                ...
                if(confirm('確定要刪除這些資料嗎？')){
                    location.href = 'member_data_delete_all.php?sids=' + sids.toString();
                }
