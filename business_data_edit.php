...
$page_name = 'business_data_edit';
$page_title = '編輯資料';

$sid = isset($_GET['sid']) ? intval($_GET['sid']) : 0;
if(empty($sid)){    //sid為0的話 跳回列表頁
    header('Location: business_list.php');
    exit;
}

$sql = "SELECT * FROM `business` WHERE `sid`=$sid";
...
<style>
            ...
            <div class="card-body">
                ...
                <form name="form1" onsubmit="return checkForm()"> <!--不透過form傳送post 透過function傳送-->
                    ...
                    <div class="form-group">
                        ...
                        <!-- htmlentities包起來 把字符轉換為HTML實體 避免特殊字元 -->
                        <small id="nameHelp" class="form-text"></small>
                    </div>
                    <div class="form-group">
                        <label for="sort">分類</label>
                        <input type="text" class="form-control" id="sort" name="sort" value="<?= htmlentities($row['sort']) ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="vat" name="vat" value="<?= htmlentities($row['vat']) ?>">
                        <small id="vatHelp" class="form-text"></small>
                    </div>
                    <div class="form-group">
                        <label for="principal">負責人</label>
                        <input type="text" class="form-control" id="principal" name="principal" value="<?= htmlentities($row['principal']) ?>">
    ...
    <script>
        ...
        function checkForm() {
            ...
            if(isPass) {
                fetch('business_data_edit_api.php',{ //傳送給php, 使用參數設定值
